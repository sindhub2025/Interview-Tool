...
def test_queued_normalized_questions_show_send_actions_and_respect_lock() -> None:
    ...
    window.set_queued_normalized_questions(
        [
            ...
            "How do you monitor SLA breaches in production?",
            "How do you prioritize incidents in a live release?",
        ]
    )
    window._reveal_question_answer_area()
    QTest.qWait(300)
...
def test_pending_normalized_segments_keep_latest_three() -> None:
    ...
    window.set_normalized_segments(
        [
            {
                "segment_id": "segment-1",
                "normalized_text": "How do you handle schema drift in production",
                "status": "pending",
            },
            {
                "segment_id": "segment-2",
                "normalized_text": "How do you validate row counts after late arriving data",
                "status": "pending",
            },
            {
                "segment_id": "segment-3",
                "normalized_text": "How do you monitor SLA breaches in production",
                "status": "pending",
            },
            {
                "segment_id": "segment-4",
                "normalized_text": "How do you prioritize incidents in a live release",
                "status": "pending",
            },
            {
                "segment_id": "segment-5",
                "normalized_text": "How do you prevent duplicate processing in production",
                "status": "pending",
            },
        ]
    )
    ...
    assert window._queued_normalized_questions == [
        "How do you monitor SLA breaches in production?",
        "How do you prioritize incidents in a live release?",
        "How do you prevent duplicate processing in production?",
    ]
    assert window._queued_question_labels[0].text() == (
        "How do you monitor SLA breaches in production?"
    )
    assert window._queued_question_labels[1].text() == (
        "How do you prioritize incidents in a live release?"
    )
    assert window._queued_question_labels[2].text() == (
        "How do you prevent duplicate processing in production?"
    )
